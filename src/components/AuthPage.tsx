...
export function AuthPage() {
  ...
  const handleCompleteSignup = async (e: React.FormEvent) => {
    ...
    try {
      const success = await signup({
        ...
        email: `${signupForm.phone.trim()}@phone.local`, // Generate email from phone for compatibility
        phone: signupForm.phone.trim(),
        language: signupForm.language || 'VI'
      });
      if (success) {
  ...
  const handleDemoLogin = async () => {
    setIsLoading(true);
    try {
      const success = await login('demo@farmassist.com', 'demo123');
      if (success) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      // For demo purposes, use phone as email equivalent
      const success = await login(loginForm.phone.trim(), loginForm.password?.trim() || 'demo');
      if (success) {
      ...
      toast.error(language === 'EN' ? 'Login failed' : 'Đăng nhập thất bại');
    } finally {
      setIsLoading(false);
    }
  };
